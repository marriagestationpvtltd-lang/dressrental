...
--
--   B) Website ADMIN LOGIN credentials (go in your .env file):
--      ADMIN_EMAIL, ADMIN_PASSWORD, ADMIN_PHONE
--      The ProductionSeeder (run automatically by .cpanel.yml)
--      reads these values from .env and creates the admin account.
--      ➜ Set them in .env, not in this SQL file.
--
--   Both sets must be configured — this file handles the schema
--   only.  Your .env file handles (A) and (B).
--
-- HOW TO USE:
...
--   4. Then edit your .env file with the MySQL connection
--      credentials from cPanel → MySQL® Databases AND set
--      ADMIN_EMAIL, ADMIN_PASSWORD, ADMIN_PHONE so the
--      ProductionSeeder can create the admin account on deploy.
--   5. Push to GitHub — cPanel auto-deploy will run
...
-- ============================================================
-- Default settings  (INSERT IGNORE — safe to re-import)
-- ============================================================
