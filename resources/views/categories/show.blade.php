...
@section('title', $category->name . ' Dresses')
        ...
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                ...
                @if($category->description)
                    <p class="text-violet-200 mt-2 text-sm md:text-base max-w-xl">{{ $category->description }}</p>
                @else
            ...
            <div class="mt-2">
                <x-share-button
                    ...
                    :title="$category->name . ' Dresses — ' . config('app.name')"
                />
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col md:flex-row gap-6">
            ...
            <aside class="md:w-64 shrink-0">
                <div class="bg-white rounded-2xl shadow-card border border-violet-100 sticky top-20 overflow-hidden">
                    ...
                    <form method="GET" action="{{ route('categories.show', $category->slug) }}" class="p-5 space-y-5">
                        <!-- Size -->
                        <div>
                        ...
                        @if(request()->hasAny(['size','min_price','max_price']))
                            <a href="{{ route('categories.show', $category->slug) }}" class="flex items-center justify-center gap-1.5 text-center text-sm text-rose-500 hover:text-rose-600 font-semibold border border-rose-200 rounded-xl py-2 hover:bg-rose-50 transition-all">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-center justify-between mb-6 bg-white rounded-2xl px-5 py-3.5 border border-violet-100 shadow-card">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-base font-extrabold text-gray-900">
                            {{ $dresses->total() }} <span class="text-gray-400 font-medium">Dresses in {{ $category->name }}</span>
                        </h2>
                    </div>
                    @if(request()->hasAny(['size','min_price','max_price']))
                ...
                @else
                    <div class="text-center py-20 bg-white rounded-3xl border border-violet-100 shadow-card">
                        <div class="w-24 h-24 bg-violet-50 border-2 border-violet-100 rounded-3xl flex items-center justify-center text-5xl mx-auto mb-5">
                            {{ $category->icon ?: '👗' }}
                        </div>
                        <h3 class="text-xl font-extrabold text-gray-900 mb-2">No Dresses Found</h3>
                        <p class="text-gray-400 mb-6 text-sm">No dresses available in {{ $category->name }} right now</p>
